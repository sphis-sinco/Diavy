...
class DTC_InputCheck
{
	...
	{
		if (FlxG.keys.justReleased.ENTER && PlayState.instance.can_press_enter) PlayState.instance.nextDialogue();
			...
			if (PlayState.instance.dialogue[PlayState.instance.dialogue_progress].choices.length > 0)
		{
			if (FlxG.keys.anyJustReleased(PlayState.instance.choices_keys))
			{
				...
				{
					if (FlxG.keys.anyJustReleased([key])) ScriptsManager.callScript(PlayState.instance.choices_events[i], [], () -> {
